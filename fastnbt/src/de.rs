...
/// Consume an i32, and parse
fn consume_i32_len<'de, In: Input<'de>>(input: &mut In, max_len: usize) -> Result<usize> {
    let len_i32 = input.consume_i32()?;
    if len_i32 < 0 {
    ...
    if len > max_len {
        return Err(Error::bespoke(format!(
            "array size ({}) greater than max sequence length ({})",
            len, max_len,
        )));
    }
        ...
        V: de::Visitor<'de>,
    {
        ...
        match self.tag {
            ...
            Tag::ByteArray => {
                ...
                let len = consume_i32_len(&mut self.de.input, self.de.opts.max_seq_len)?;
